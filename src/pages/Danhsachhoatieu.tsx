...
const Danhsachhoatieu: React.FC = () => {
  ...
  const [selectedHoatieu, setSelectedHoatieu] = useState<HoaTieu | null>(null);
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {selectedHoatieu && (
          <motion.div
            className="hoatieu-modal-overlay"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            onClick={() => setSelectedHoatieu(null)}
          >
            <motion.div
              className="hoatieu-modal"
            ...
            >
              <img
                src={`${SERVER}/${selectedHoatieu.image}`}
                alt={selectedHoatieu.name}
              />
              <h2>{selectedHoatieu.name}</h2>
              <h4>{selectedHoatieu.rank}</h4>
              <p>{selectedHoatieu.content}</p>
              <p>{selectedHoatieu.content_en}</p>
              <button onClick={() => setSelectedHoatieu(null)}>Đóng</button>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
