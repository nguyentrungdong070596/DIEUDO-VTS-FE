  ...
  const loadHoatdongcongty = async () => {
    try {
      ...
      if (response.data && Array.isArray(response.data.data)) {
        ...
        const dataWithHeight = response.data.data.map(
          (item: HoatDongCongTy, index: number) => ({
            ...item,
        ...
        <div className="row">
          ...
          <div className="col-custom m-12 c-12 l-9">
            ...
            <div className="animate__animated animate__fadeInUp detail-hoatdong">
              <img
                src={`${SERVER}/${hoatdongItem.image}`}
                alt={hoatdongItem.title}
              />
              {hoatdongItem.videourl && (
                <div className="detail-hoatdong-video">
                  ...
                  >
                    <source
                      src={`${SERVER}/${hoatdongItem.videourl}?v=${Date.now()}`}
                      type="video/mp4"
                    />
              ...
              <p
                dangerouslySetInnerHTML={{
                  __html: t(`content_hoatdongcongty_${hoatdongItem.id}`) || "",
                }}
              ></p>
              ...
              <span className="detail-hoatdong-postdate">
                {t("ngay")}:{" "}
                {new Date(hoatdongItem.postdate).toLocaleDateString("vi-VN")}
              </span>
            </div>
          </div>
        </div>
