...
import { useState } from "react";
import Header from "./layout/Header";
import { HashRouter, Route, Routes } from "react-router-dom"; // 🔁 Đổi BrowserRouter thành HashRouter
import Home from "./pages/Home";
import Footer from "./layout/Footer";
import Kehoachdantau from "./pages/Kehoachdantau";
import Giodieudong from "./pages/Giodieudong";
...
function App() {
  ...
  return (
    <div style={{ zoom: "90%" }}>
      <HashRouter>
        {" "}
        {/* ✅ Đã đổi BrowserRouter thành HashRouter */}
        <SearchContext.Provider value={{ keyword, setKeyword }}>
          <ScrollToTop />
          <Header />
          <Routes>
            ...
            <Route path="/tin-tuc/detail/:id" element={<Newsdetail />} />
            <Route
              path="/danh-sach-phuong-tien"
              element={<Danhsachphuongtien />}
            />
            <Route
              path="/gia-dich-vu/detail/:id"
              element={<GiadichvuDetail />}
            />
            <Route path="/dich-vu/detail/:id" element={<ServiceDetail />} />
            <Route path="/he-thong-cang-bien" element={<Hethongcangbien />} />
            <Route path="/tuyen-luong" element={<Tuyenluong />} />
            <Route path="/tuyen-luong-thi-vai" element={<Tuyenluongthivai />} />
            <Route
              path="/tuyen-luong-song-dinh"
              element={<Tuyenluongsongdinh />}
            />
            <Route
              path="/vi-tri-don-tra-hoa-tieu"
              element={<Vitridontrahoatieu />}
            />
            <Route path="/gioi-thieu-cong-ty" element={<Gioithieucongty />} />
            <Route path="/lich-thuy-trieu" element={<Lichthuytrieu />} />
            <Route path="/hoat-dong-cong-ty" element={<Hoatdongcongty />} />
            <Route
              path="/hoat-dong-cong-ty/detail/:id"
              element={<HoatdongcongtyDetail />}
            />
          </Routes>
          <Footer />
          <GoToTopButton />
